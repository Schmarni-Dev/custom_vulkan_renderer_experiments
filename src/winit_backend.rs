...
impl Backend for WinitBackend {
    ...
    fn required_instance_exts(init: &Self::Init) -> vulkano::instance::InstanceExtensions {
        Surface::required_extensions(init).unwrap()
    }

    fn required_device_exts(_init: &Self::Init) -> vulkano::device::DeviceExtensions {
    ...
    fn required_device_features(_init: &Self::Init) -> vulkano::device::DeviceFeatures {
        vulkano::device::DeviceFeatures::empty()
    }

    fn supports_queue_type(
        init: &Self::Init,
        phys_dev: &Arc<vulkano::device::physical::PhysicalDevice>,
        type_index: u32,
    ) -> bool {
        phys_dev
            .presentation_support(type_index, init)
            .unwrap_or(false)
    }

    fn create(init: Self::Init, renderer: std::sync::Arc<crate::renderer::Renderer>) -> Self {
        Self {
            renderer,
            render_reqs: None,
            a: 0.0,
            recreate_swapchain: false,
            event_loop: Some(init),
        }
    }
}
impl WinitBackend {
